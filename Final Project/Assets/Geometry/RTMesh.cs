...
public class RTMesh : RTGeometry
{
    ...
    public List<RTTriangle_t> GetGeometry()
    {
        ...
        List<RTTriangle_t> output = new List<RTTriangle_t>(triangles.Length / 3);

        for (int i = 0; i < triangles.Length; i += 3)
        {
            output.Add(GenerateTriangle(vertices[triangles[i]], vertices[triangles[i + 1]], vertices[triangles[i + 2]]));
        }
